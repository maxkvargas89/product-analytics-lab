...
# CONFIGURATION
file_path = "/Users/maxvargas/code_testing/data/ab_test_homepage_headline_data.csv"
ab_test_file = pd.read_csv(file_path)
ab_test_file_ready = ab_test_file.groupby('group').agg(
    conversions=('signed_up','sum'),
    total_users=('user_id','count'))
ab_test_file_ready = ab_test_file_ready.reset_index() # A is control / B is test

LOG_PATH = "/Users/maxvargas/code_testing/data/ab_test_homepage_headline_log.txt"
ALPHA = 0.05
print(ab_test_file_ready.head(10))
...
# Function to visualize AB results 
def plot_ab_results(df, results):
    # Bar chart of observed conversion rates
    conv_rates = {
        'Group A': results['conversion_rate_A'],
        'Group B': results['conversion_rate_B']
    }

    plt.figure(figsize=(6, 4))
    sns.barplot(x=list(conv_rates.keys()), y=list(conv_rates.values()), palette='Set2')
    ...
    # Optional: Display p-value and conclusion
    print(f"\nZ-Test Result:")
